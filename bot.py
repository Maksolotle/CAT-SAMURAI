...
async def cmd_boo(message: types.Message):
    responses = [
        "Лучше принеси мне пивка", "Опять мешают работать!",
        "Ну и зачем ты это сказал?😤", "Так и до инфаркта можно довести!"
    ]
...
async def cmd_report(message: types.Message):
    user_reports = [
        "Щя кто-то чекнет :3", "Жалоба отправлена на рассмотрение",
    ...
    try:
        chat_member = await message.bot.get_chat_member(
            message.chat.id, reported_user.id)
        if chat_member.status in ['administrator', 'creator']:
            await message.reply(random.choice(admin_reports))
            return
    except Exception as e:
        print(f"Error retrieving user data: {e}")

    current_time = datetime.now().strftime("%d.%m.%Y %H:%M")
    await message.reply(random.choice(user_reports))

    log_message = (
        f"<b>[REPORT]</b>\n"
        f"🕒 <b>Time: </b> {current_time}\n"
        f"👤 <b>Reporter: </b> {reporter_user.full_name} ({reporter_user.id})\n"
        f"🚩 <b>Reported: </b> {reported_user.full_name} ({reported_user.id})\n"
        f"💬 <b>Message: </b> {message.reply_to_message.text or 'media/other'}")
...
async def handle_message(message: types.Message):
    ...
    #else:
    #await message.reply('Сообщение прошло проверку')
